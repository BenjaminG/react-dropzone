...
class Dropzone extends React.Component {
  ...
  static resetImageOrientation(image) {
    return new Promise(
  ...
  onDrop(evt) {
    const { onDrop, multiple, disablePreview, accept, maxSize, minSize } = this.props;
    ...
    if (multiple) {
      Promise.all(fileList.map(Dropzone.resetImageOrientation)).then((blobs) => {
        blobs.forEach((blob, index) => {
          if (!disablePreview) {
            fileList[index].preview = blob; // eslint-disable-line no-param-reassign
          }
          if (
            fileAccepted(fileList[index], accept) &&
            fileMatchSize(fileList[index], maxSize, minSize)
          ) {
            acceptedFiles.push(fileList[index]);
          } else {
            rejectedFiles.push(fileList[index]);
          }
        });
